...
def link_scraper(t):
    ...
    for link in links:
        ...
        if "url?q=" in link_href and not "webcache" in link_href:
            ...
            if len(title) > 0:
                try:
                    praw_comments(link.get('href').split("?q=")[1].split("&sa=U")[0], alpha)
                except:
...
def praw_comments(link, dict): #should return a list of objects
    comments_list = []
    ...
    dict[submission] = comments_list
    return  #return a list of all comment objects
